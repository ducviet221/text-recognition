...
box1 = np.array([1110, 473, 1493, 481, 1491, 599,1107 ,591])
box2 = np.array([418,224,500,231,500,258,416,254])
...
def iou_score(box1, box2):
    ...
    if any(
        cv2.contourArea(np.array(box, dtype="int")[:, np.newaxis, :]) == 0
        for box in [box1, box2]
    ):
        warnings.warn("A box with zero area was detected.")
        return 0
    pc = pyclipper.Pyclipper()
    pc.AddPath(np.array(box1, dtype="int"), pyclipper.PT_SUBJECT, closed=True)
    pc.AddPath(np.array(box2, dtype="int"), pyclipper.PT_CLIP, closed=True)
    ...
    union = sum(
        cv2.contourArea(np.array(points, dtype="int")[:, np.newaxis, :])
